...
export const StudentApp = ({ state, onBack }: StudentAppProps) => {
  ...
  return (
      <div className="fixed inset-0 flex flex-col bg-slate-50 overflow-hidden font-sans">
          ...
          <div className="fixed top-20 right-4 w-32 h-24 bg-black rounded-lg shadow-xl overflow-hidden z-30 border-2 border-slate-800 group">
              <video 
                  ref={videoRef} 
                  autoPlay 
                  playsInline 
          ...
          <div className="bg-[#0f1d2e] text-white h-14 flex justify-between items-center px-4 shadow-md flex-shrink-0 z-20">
              ...
              <div className="flex gap-2">
                 <div className="bg-slate-800 px-2 py-1 rounded font-mono text-xs border border-slate-700 text-emerald-400 flex items-center gap-1">
                    <Wifi size={10}/> {sessionMode === 'LIVE_REAL' ? 'Online' : 'Local'}
                 </div>
                 {isKioskActive && <div className="bg-emerald-900 px-2 py-1 rounded font-mono text-xs text-emerald-300 border border-emerald-700">Kiosk ON</div>}
              </div>
          </div>
